...
- [ ] **1.0.5. Création de la structure du sous-dossier ai_agent (Nouveau)**
    - [ ] Créer le dossier `app/ai_agent/`
    - [ ] Créer `app/ai_agent/__init__.py`
    - [ ] Mettre à jour les imports dans les autres modules si nécessaire pour refléter la nouvelle structure.

- [ ] **1.0.6. Harmonisation des versions Gemini (Nouveau)**
    - [ ] Confirmer que la version finale de Gemini à utiliser est `gemini-2.5-flash-preview-05-20`.
    - [ ] Mettre à jour `app/config.py` (champ `GEMINI_MODEL_NAME`) pour utiliser cette version (Vérifié, déjà sur `gemini-2.5-flash-preview-05-20`).
    - [ ] Mettre à jour `.env.example` (variable `GEMINI_MODEL_NAME`) pour utiliser cette version (Action manuelle requise par l'utilisateur car accès direct non possible).
...
-   [ ] **1.10. `app/main.py` (Backend FastAPI - Modifications pour UI)**
    -   [ ] **1.10.1. Intégration Socket.io (FastAPI)**:
        ...
        -   [ ] **Installer `python-socketio[fastapi]`** (ou vérifier que `python-socketio` seul suffit avec l'intégration FastAPI de `SocketManager`).
        -   [ ] **Créer `app/socket_manager.py` avec une classe `SocketManager`**: 
            -   [ ] Initialiser `fastapi_sio = socketio.ASGIApp(sio_server=socketio.AsyncServer(async_mode='asgi', cors_allowed_origins="*"))`.
            -   [ ] Exposer `fastapi_sio` pour être monté dans `app/main.py`.
            -   [ ] Définir des méthodes dans `SocketManager` pour émettre des événements de manière centralisée (ex: `async def emit_bot_status(data)`, `async def emit_portfolio_update(data)`).
        -   [ ] Configurer `SocketManager` dans FastAPI (monter `fastapi_sio` dans `app/main.py`).
        -   [ ] Implémenter des namespaces et **événements Socket.io de base** pour envoyer des données en temps réel à l'UI React (ex: mises à jour de l'état du bot, P&L, logs, décisions IA, santé des systèmes). (Payloads à définir en cohérence avec `todo/01-todo-ui.md Phase 4.1`).
            -   [ ] Événement `connect`: Gestion de la connexion client (log serveur, potentiellement infos utilisateur).
            -   [ ] Événement `disconnect`: Gestion de la déconnexion client (log serveur).
            -   [ ] Événement `bot_status_update`: Pour envoyer le statut actuel du bot (ex: RUNNING, STOPPED, PAUSED, ERROR, current_cycle, next_cycle_in_sec).
            -   [ ] Événement `portfolio_update`: Pour envoyer les mises à jour du portefeuille (ex: total_value_usd, pnl_24h_usd, positions, available_cash_usdc).
            -   [ ] Événement `new_trade_executed`: Pour notifier un nouveau trade (ex: trade_id, pair, type, amount, price, status, reason_source, ai_reasoning_id).
            -   [ ] Événement `ai_agent_decision`: Pour envoyer la dernière décision de l'IA (ex: decision_id, decision_type, token_pair, confidence, reasoning_snippet).
            -   [ ] Événement `market_data_update`: Pour envoyer des mises à jour de données de marché (ex: pair, current_price, volume_24h_usd, sentiment_score).
            -   [ ] Événement `system_health_update`: Pour envoyer l'état de santé des composants (ex: service_name, status, last_error).
            -   [ ] Événement `new_log_entry`: Pour streamer les logs (ex: level, module, message, timestamp_utc).
        -   [ ] Définir des événements pour recevoir des commandes de l'UI (ex: Start/Stop Bot, ajustement des paramètres de stratégie) et les router vers `DexBot` ou les services concernés.
        -   [ ] **Créer un middleware d'authentification Socket.io** (ou un gestionnaire d'événement `connect` avec authentification) pour sécuriser la connexion WebSocket, utilisant les tokens JWT de Clerk/Auth0. Rejeter les connexions non authentifiées.
